...
JoyTeleop::JoyTeleop() {
    ...
    //These are loaded from the amee_control.yaml by the control.launch file.
    nh.getParam("/amee_velocity_controller/linear/x/max_velocity", max_velocity);
    nh.getParam("/amee_velocity_controller/angular/z/max_velocity", max_angular);
    ...
    sled_state_sub = nh.subscribe<control_msgs::JointControllerState>("/amee_sled_position_controller/state", 10, &JoyTeleop::sledStateCallback, this);

    //wait for most recent set_point
    ros::topic::waitForMessage<control_msgs::JointControllerState>("/amee_arm_position_controller/state");
    ros::topic::waitForMessage<control_msgs::JointControllerState>("/amee_bucket_position_controller/state");
    ros::topic::waitForMessage<control_msgs::JointControllerState>("/amee_sled_position_controller/state");
